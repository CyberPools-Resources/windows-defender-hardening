...
<#
.SYNOPSIS
    Hardens native Windows Defender (built-in antimalware) to maximum protection settings.

.DESCRIPTION
    This script configures Windows Defender to its most aggressive security posture
    using only the built-in, free antimalware engine included with Windows 10/11.
    No paid licenses or additional products are required.

    Designed for deployment during active security incidents where immediate endpoint
    protection is critical. All settings use the native Set-MpPreference cmdlet and
...
.PARAMETER IncidentMode
    When set to $true (default), uses the most aggressive settings including
    CloudBlockLevel ZeroTolerance. Set to $false for post-incident steady-state
    configuration that balances security with fewer false positives.
#>
...
Write-Host " Windows Defender Hardening Script v1.0" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

if ($IncidentMode) {
    Write-Host "[MODE] INCIDENT RESPONSE - Maximum aggression enabled" -ForegroundColor Red
} else {
    Write-Host "[MODE] STEADY STATE - Balanced security/usability" -ForegroundColor Yellow
}
Write-Host ""
...
# -------------------------------------------------------------------
# STEP 2: Core protection settings
# -------------------------------------------------------------------
...
$cloudBlockLevel = if ($IncidentMode) { 6 } else { 2 }  # 6=ZeroTolerance, 2=High
$cloudLevelName  = if ($IncidentMode) { "ZeroTolerance (6)" } else { "High (2)" }
...
$cloudParams = @{
    ...
    CloudExtendedTimeout    = 50         # +50 seconds cloud analysis (60 total)
}
...
try {
    ...
    Write-Host "  - Cloud Block Level: $cloudLevelName" -ForegroundColor White
    Write-Host "  - Cloud Extended Timeout: 60 seconds total" -ForegroundColor White
} catch {
...
$asrRules = @{
    ...
    # Block executable files unless prevalence/age/trusted list
    "01443614-cd74-433a-b99e-2ecdc07bfc25" = "Block unknown executables (cloud reputation)"
...
$auditIfSCCM = "d1e49aac-8f56-4280-b9ba-993a6d77406c"  # PSExec/WMI - breaks SCCM

$ruleIds = @($asrRules.Keys)
$ruleActions = @($asrRules.Keys | ForEach-Object { 1 })  # 1 = Block for all rules

# Add PSExec/WMI rule in Audit mode (safe default, won't break SCCM)
$ruleIds += $auditIfSCCM
$ruleActions += 2  # 2 = Audit
...
try {
    ...
    foreach ($guid in $asrRules.Keys) {
        Write-Host "  - BLOCK: $($asrRules[$guid])" -ForegroundColor White
    }
    Write-Host "  - AUDIT: Block PSExec/WMI process creation (set to Audit to avoid breaking SCCM)" -ForegroundColor Yellow
} catch {
...
$cfaValue = if ($IncidentMode) { 1 } else { 2 }  # 1=Enabled, 2=AuditMode
$cfaName  = if ($IncidentMode) { "ENABLED (Block)" } else { "AUDIT MODE" }

try {
    Set-MpPreference -EnableControlledFolderAccess $cfaValue
    Write-Host "  - Controlled Folder Access: $cfaName" -ForegroundColor White
    if ($IncidentMode) {
...
$scanParams = @{
    ...
    ScanAvgCPULoadFactor                            = 70         # Higher CPU during incident
...
try {
    ...
    Write-Host "  - CPU limit during scan: 70%" -ForegroundColor White
} catch {
...
$sigParams = @{
    SignatureUpdateInterval                          = 1          # Every 1 hour
...
try {
    ...
    Write-Host "  - Signature check interval: EVERY HOUR" -ForegroundColor White
...
$threatParams = @{
    ...
    QuarantinePurgeItemsAfterDelay      = 0          # Never auto-delete quarantined items
...
try {
    ...
    Write-Host "  - All threat levels: QUARANTINE (preserved for analysis)" -ForegroundColor White
    Write-Host "  - Quarantine purge: NEVER (keep all samples)" -ForegroundColor White
...
# -------------------------------------------------------------------
# STEP 10: Exploit Protection (system-wide mitigations)
# -------------------------------------------------------------------
...
Write-Host "  - Cloud protection (MAPS Advanced, $cloudLevelName)" -ForegroundColor White
Write-Host "  - Network protection (malicious domain blocking, traffic inspection)" -ForegroundColor White
Write-Host "  - 17 ASR rules in Block mode + 1 in Audit mode" -ForegroundColor White
Write-Host "  - Controlled Folder Access: $cfaName" -ForegroundColor White
Write-Host "  - Aggressive scan schedule (daily full + noon quick)" -ForegroundColor White
Write-Host "  - Hourly signature updates" -ForegroundColor White
...
Write-Host "  3. After incident: Switch to steady-state mode by running:" -ForegroundColor White
Write-Host "     .\Harden-WindowsDefender.ps1 -IncidentMode:`$false" -ForegroundColor Gray
Write-Host "  4. A reboot is recommended but not required for most settings to take effect" -ForegroundColor White
Write-Host ""
